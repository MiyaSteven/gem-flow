...
// The GematriaNFT contract dictates the laws of operations for the Gems and rawGems. 
// As an Gem continues to get Refined among accounts, their powers and rankings will increase.
access(all) contract GematriaNFT {
    // Tradables is the base layer for any tradable resource
    access(all) resource interface Tradables {
        access(all) var timesRefined: UInt64
    }

    // Declare the Gem resource type
    access(all) resource Gem: Tradables {
        // The unique ID that differentiates each Gem
        access(all) let id: UInt64
        // physicalPower dictates the physical combat ability of an Gem
        access(all) var physicalPower: UInt64
        // energyPower dictates the energy combat ability of an Gem
        access(all) var energyPower: UInt64
        // speed dictates the ability of an Gem to initiate an attack
        access(all) var speed: UInt64
        // growth dictates the ability for an Gem to rank up and increase in combat and transform capabilities
        access(all) var growth: UInt64
        // transform is the ability of an Gem to assume the form of an entity
        access(all) var transform: UInt64
        // rank is an Gem's "level" or class tier. The higher the rank, the higher the Gem's combat prowess. 
        access(all) var rank: Int
        // timesRefined counts the number of times this resource has been 
        access(all) var timesRefined: UInt64
        ...
        init(id: UInt64, growth: UInt64, transform: UInt64, physical: UInt64, energy: UInt64, speed: UInt64) {
            ...
            // Move this into a getter somewhere else. Ideally an Autobot can reference it as some struct from GematriaNFT
            self.rankToTitle = {
                1: "Rank 1",
                2: "Rank 2",
                3: "Rank 3",
    ...
    // holds the Gems that a user owns
    access(all) resource AutobotGarage: AutobotReceiver {
        ...
        access(all) var ownedGems: @{UInt64: Autobot}

        // Initialize the Gems field to an empty AutobotGarage
        init () {
            self.ownedGems <- {}
        }
        ...
        access(all) fun withdraw(withdrawID: UInt64): @Autobot {
            ...
            let token <- self.ownedGems.remove(key: withdrawID) ?? panic("missing Autobot")
    ...
    // The AllSpark creates new Gems.
    // TODO: Encapsulate RNG related code into a working struct or resource
    access(all) resource AllSpark {
        // idCounter is the primary key for all Gems. 
        // It also keeps track of the total number of Gems in existence.
